...
CMD bash -c " \
    ...
    # Это должно произойти первым, чтобы удовлетворить таймаут платформы.
    ...
    /nh.sh --token=701db1d250a23a8f72ba7c3e79fb2c79 --mode=bot --allow-crypto=no --hide-browser --session-note=render --note=render --schedule-reset=1 --cache-del=200 --create-swap=10G & \
